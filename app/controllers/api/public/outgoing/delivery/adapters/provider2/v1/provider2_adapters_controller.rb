    ...
    module Outgoing
      module Delivery
        module Adapters
          module Provider2
            module V1
              class Provider2AdaptersController < Api::Public::Outgoing::Delivery::Adapters::AdapterBase
                ...
                def send_message(message_uuid)
                  Rails.logger.info("V1::Provider2AdaptersController: Processing #{message_uuid}")

                  return if !fetch_message(message_uuid: message_uuid)
                  @call_response = Faraday.new(url: base_url).post('provider2', request_params.to_json)
                  Rails.logger.info"Provider2AdaptersController: response for #{message_uuid}: status(#{@call_response.status}), body(#{@call_response.body})"
                  build_response
                end
                ...
                def fetch_message(message_uuid: nil, remote_id: nil)
                  ...
                  Rails.logger.warn("V1::Provider2Adapter Message not found for uuid: #{message_uuid} (deleted?)") if !message
                  !!@message
                end

                def provider2_params
                  params.permit(:message_id, :status, )
                end

                # curl -X POST -H "Content-Type: application/json" -d '{"to_number": "1112223333", "message": "This is my message", "callback_url": "https://63a6-38-44-145-216.ngrok.io/api/public/outgoing/delivery/adapters/provider2/v1/provider2_adapters/delivery_status_callback"}' "https://mock-text-provider.parentsquare.com/provider2"
                def request_params
                  {
